...
namespace RenderHeads.Media.AVProDeckLink
{
    ...
    public class DeckLinkOutput : DeckLink
    {
		...
		private RenderTexture _lToGTexture;
		...
		private void InitCaptureBlendResources(int width, int height)
		{
			...
			if (DeckLinkSettings.Instance._multiOutput)
			{
				...
				for(int i = 0; i < 2; ++i)
				{
					_capturedFrames[i] = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default, (int)_antiAliasingLevel);
				}
			}

			if(_blended == null)
			{
				_blended = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default, (int)_antiAliasingLevel);
			}
			...
			if (_inputTexture == null)
			{
				_inputTexture = RenderTexture.GetTemporary(width, height, 24, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default, (int)_antiAliasingLevel);
			}
        ...
        private void InitColorspaceResources(int inputTexWidth, int inputTexHeight, RenderTextureFormat format)
        {
            if(QualitySettings.activeColorSpace == ColorSpace.Linear)
            {
                _lToGTexture = RenderTexture.GetTemporary(inputTexWidth, inputTexHeight, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1);
                _lToGShader = Shader.Find("AVProDeckLink/LinearToGamma");
                _lToGMat = new Material(_lToGShader);
            }
            else
            {
                _lToGMat = null;
        ...
        private void InitInterlaceResources(int width, int height)
        {
			...
			if (_interlacedTexture == null)
            {
                _interlacedTexture = RenderTexture.GetTemporary(width, height, 0);
		...
		protected override void BeginDevice()
		{
			...
			else
			{
				...
				InitColorspaceResources(_inputTexture.width, _inputTexture.height, RenderTextureFormat.ARGB32);
		...
		private RenderTexture ConvertColourSpace(RenderTexture inputTexture)
		{
			if (QualitySettings.activeColorSpace == ColorSpace.Linear)
			{
				Graphics.Blit(inputTexture, _lToGTexture, _lToGMat);
				return _lToGTexture;
			}
